...
import { EventStreamContentType, getLines, getBytes, getMessages } from './event-source-parse'
        ...
        reject(
            createError(
                `Request failed with status code ${response.status} and body ${
                    typeof response.data === 'string' ? response.data : tryJsonStringify(response.data)
                }`,
                response.config,
                // @ts-ignore
                null,
